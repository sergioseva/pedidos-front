...
What gets stored is the rule that drives the whole lifecycle: an empty or finalized remito **erases** its draft, so saving and starting over clean up on their own with no extra code. That rule needs `skip(1)` on the subscription and it is not optional: a `BehaviorSubject` replays its seed the moment anyone subscribes, and the seed is an empty remito — without skipping it, constructing the service (that is, loading the page) read as "the remito was emptied" and wiped the draft before anything could restore it. "Empty erases" may only mean a real emptying. Drafts are keyed by `re_tipo`, otherwise walking into the consignment screen would restore a devolución.
